...
if isWithZ:
    for s in Sessions:
        for c in Slots:
            ...
            # If z_var is 1, all x_vars must be 0
            for x_var in x_vars:
                mdl.add_constraint(x_var + z_var  <= 1 )
            # If z_var is 0, at least one x_var must be 1
            mdl.add_constraint(mdl.sum(x_vars) + z_var >= 1)
...
if data_set_choice == "2024":
    ### note le for after if 
    if isWithZ:
        for c in [1, 2, 3, 4]:
            mdl.add_constraint(z[34, c] == 1)
    else:
        for c in [1, 2, 3, 4]:
            for l in PaperRangeIndex:
